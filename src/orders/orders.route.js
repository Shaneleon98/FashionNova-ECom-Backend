...
router.post("/create-checkout-session", async (req,res) => {
    ...
    try {
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            mode:'payment',
            success_url: `https://fashion-nova-frontend-final-y8dj.vercel.app/success?session_id={CHECKOUT_SESSION_ID}`,
            cancel_url: `https://fashion-nova-frontend-final-y8dj.vercel.app/cancel`,
...
router.get("/", async (req, res) => {
   ...
   try {
     ...
     if(orders.length === 0){
        return res.status(404).send({message: "Order not found", orders: [] });
     }
     res.status(200).send(orders)
   } catch (error) {
